...
SpawnLocalVehicle = function(data)
	...
	local spawnpoint = Configgarage.Garages[cachedData["currentGarage"]]["positions"]["vehicle"]
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		esx.ShowNotification('Veuillez déplacer le véhicule hors de la route', 1)
...
SpawnVehicle = function(data,recuperar)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		esx.ShowNotification('Veuillez déplacer le véhicule hors de la route', 2)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Configgarage.Trim(GetVehicleNumberPlateText(vehicle)) == Configgarage.Trim(vehicleProps["plate"]) then
                esx.ShowNotification('Votre véhicule est déjà dans les rues de la ville', 2)
    ...
    if recuperar then
        esx.ShowNotification('Vous avez payé 200 $ pour récupérer votre véhicule!', 1)
    ...
    if DoesEntityExist(vehicle) then
		...
		esx.TriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
                ...
                CloseMenu()
                esx.ShowNotification('Ce véhicule ne vous appartient pas', 2)
			end
...
SaveInGarage = function(garage)
    ...
    esx.ShowNotification('Vous avez enregistré votre véhicule au garage '..garage, 3)
    Citizen.Wait(500)
...
HandleCamera = function(garage, toggle)
    ...
    local Camerapos = Configgarage.Garages[garage]["camera"]
...
DrawScriptMarker = function(markerData)
    DrawMarker(36, markerData["pos"],0,0,0,0,0,0,1.0,1.0,1.0,112, 0, 198, 0.05,0, 0,0,0,0,0,0)
		esx.ShowHelpNotification("~p~E ~w~ou ~p~ENTRER ~w~= Accepter | ~p~Flèches ~w~= Déplacer | ~p~Retour arrière ~w~= Quitter")
end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return esx.ShowNotification('Ce modèle n\'existe pas dans le jeu, faites report !', 2)
    end
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Je te cherche " .. GetDisplayNameFromVehicleModel(model) .. "...", 255, 255, 255, 150)
	end
end